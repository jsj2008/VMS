...
-(void)drawShadowInBounds:(NSRect)bounds {
    ...
    [shadow lineToPoint:NSMakePoint(bounds.size.width, 2)];
    [shadow stroke];
...
-(void)drawProgressWithBounds:(NSRect)frame
{
    ...
    @catch (NSException *exception) {
        NSLog(@"LBProgressBar Exception: %@",exception);
        exit(0);
    }
    @finally {
        //Do nothing
...
-(void)drawRect:(NSRect)dirtyRect {
    ...
    @try {
        ...
        @autoreleasepool {
            ...
            if (value) {
                CGFloat width = self.frame.size.width*value-2*DEFAULT_inset;
                width = width < 0.0? 1.0 : width;
                NSRect bounds = NSMakeRect(DEFAULT_inset, DEFAULT_inset, width, (self.frame.size.height-2*DEFAULT_inset)-1);
...
{
    if (!_shadowColor) {
        _shadowColor = [NSColor colorWithCalibratedRed:223.0/255.0 green:238.0/255.0 blue:181.0/255.0 alpha:1.0];
    }
