...
int bs_read( bs_t *s, int i_count )
{
	 static int i_mask[33] ={0x00,
                                  ...
                                  0x1fffffff,0x3fffffff,0x7fffffff,static_cast<int>(0xffffffff)};
	/*
			  数组中的元素用二进制表示如下：
...
#define NAL_LEN 4 //"00 00 01" or "00 00 00 01"

int video_frame_type(unsigned char* video_data, int data_len)
{
	unsigned char nal_type = video_data[NAL_LEN]&0x1f;
	switch(nal_type)
	{
	case 7: //sps
		return VIDEO_SPS_FRAME;
	case 8://pps
		return VIDEO_PPS_FRAME;
	case 5: //i frame		
	case 1://b or p frame
		{
			bs_t s;
			bs_init( &s, video_data+NAL_LEN+1, data_len-NAL_LEN-1);			
			/* i_first_mb */
			bs_read_ue( &s );
			/* picture type */
